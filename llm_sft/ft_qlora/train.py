...
if VAL_SET_SIZE > 0:
    train_val = data["train"].train_test_split(test_size=32, shuffle=True, seed=618)
    generate_prompt(train_val["test"][0], is_logger=True)
...
trainer = CustomTrainer(
        ...
        gradient_accumulation_steps=GRADIENT_ACCUMULATION_STEPS,
            ...
            # eval_steps=320,  #  200 if VAL_SET_SIZE > 0 else None,
            # eval_steps=16,  # 200 if VAL_SET_SIZE > 0 else None,
            save_strategy="steps",
            save_total_limit=12,
            ...
            optim="paged_adamw_32bit",  # "adamw_torch",  # "adamw_torch",  # "adamw_hf",
            weight_decay=5e-4,
            # fp16=True
        )
    )
...
"""
# nohup python train.py > tc.ft_qlora.log 2>&1 &
# tail -n 1000  -f tc.ft_qlora.log
# |myz|

query_states = self.q_proj(hidden_states).view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
File "site-packages/peft-0.4.0.dev0-py3.8.egg/peft/tuners/lora.py", line 790, in forward
